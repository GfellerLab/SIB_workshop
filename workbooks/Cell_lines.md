...
## 2.1 Standard downstream analysis
...
</span> `sc <- NormalizeData(sc, verbose=FALSE) %>%
FindVariableFeatures(selection.method = "disp", nfeatures = 1000,
verbose=FALSE) %>% ScaleData(verbose=FALSE) %>% RunPCA(verbose=FALSE)`
...
![](Cell_lines_files/figure-gfm/Normalize%20data%20and%20compute%20a%20set%20of%20highly%20variable%20genes-1.png)<!-- -->

#### 2.1.1.2 Scaling and dimensionality redution
...
![](Cell_lines_files/figure-gfm/unnamed-chunk-2-1.png)<!-- -->

### 2.1.2 UMAP (non-linear dimensionality reduction)
...
![](Cell_lines_files/figure-gfm/UMAP-1.png)<!-- -->
...
![](Cell_lines_files/figure-gfm/unnamed-chunk-3-1.png)<!-- -->

### 2.1.4 Differential expression analysis

#### 2.1.4.1 Find Markers of cell lines
...
#### 2.1.4.2 Plot the expression of some found markers

``` r
VlnPlot(sc, features = sc.top.markers$gene[c(seq(1, 9, 2), seq(2, 10, 2))], ncol = 5, pt.size = 0.0)
```

![](Cell_lines_files/figure-gfm/unnamed-chunk-5-1.png)<!-- -->

# 3 Data simplification (coarse-graining) – Construction of *metacells*
